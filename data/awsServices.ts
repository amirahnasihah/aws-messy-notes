        ...
        services: [
          {
            ...
            ingat: '"Kawasan perumahan sendiri dalam AWS"',
            gunaUntuk: 'Isolated private network for your AWS resources',
            fungsi: 'Mencipta network peribadi yang terpencil dalam AWS. Kau tentukan sendiri IP range (CIDR), buat subnets, configure route tables dan gateways. Default VPC dah ada dalam setiap region.',
            contohGuna: 'Semua EC2, RDS, Lambda dalam kawasan terpencil kau sendiri — orang lain tak boleh access kecuali kau benarkan.',
            storageDetails: 'VPC CIDR → IP range keseluruhan VPC (contoh 172.16.0.0/16 = 65,536 IPs)\nPublic Subnet → Ada route ke Internet Gateway. EC2 boleh dapat public IP\nPrivate Subnet → Tiada route terus ke internet. DB, app servers letak sini\nAvailability Zone → Setiap subnet duduk dalam satu AZ sahaja',
            ...
            keywords: ['VPC', 'CIDR', 'subnet', 'public', 'private', 'isolated network', 'default VPC'],
          },
          {
            shortName: 'CIDR & Subnets',
            fullName: 'IP Addressing & Subnet Calculator',
            ingat: '"2^(32-prefix) = jumlah IPs, tolak 5 AWS reserved"',
            gunaUntuk: 'Plan IP address ranges for VPC and subnets',
            fungsi: 'CIDR (Classless Inter-Domain Routing) menentukan berapa banyak IP addresses dalam sesebuah network. Formula: 2^(32 - prefix) = total IPs. AWS reserve 5 IPs setiap subnet: .0 (network), .1 (router), .2 (DNS), .3 (future), .255 (broadcast).',
            contohGuna: '192.168.0.0/26 → 32-26=6, 2^6=64 IPs, tolak 5 = 59 usable. VPC /16 boleh dibahagi kepada banyak subnets /24 atau /26.',
            storageDetails: '/16 → 2^16 = 65,536 IPs (besar, untuk VPC)\n/24 → 2^8 = 256 IPs → 251 usable\n/26 → 2^6 = 64 IPs → 59 usable\n/27 → 2^5 = 32 IPs → 27 usable\n/28 → 2^4 = 16 IPs → 11 usable (minimum AWS)',
            detailsLabel: 'CIDR Reference',
            scenario: '"Exam soal berapa usable IPs dalam /27?" → 32 total, tolak 5 AWS reserved = 27 usable. Ingat: AWS ALWAYS reserves 5 IPs per subnet.',
            keywords: ['CIDR', 'subnet mask', 'IP addressing', '/24', '/26', '5 reserved IPs', 'usable hosts'],
          },
          {
            shortName: 'Internet Gateway',
            fullName: 'VPC Internet Gateway (IGW)',
            ingat: '"Pintu pagar kawasan — dua arah, free"',
            gunaUntuk: 'Connects VPC to internet (bidirectional)',
            fungsi: 'Komponen yang membolehkan komunikasi dua arah antara VPC dan internet. Highly available, horizontally scaled, free to use. Satu VPC = satu IGW. Public subnet EC2 perlu IGW + public IP + route table pointing to IGW.',
            contohGuna: 'Subnet public dengan EC2 web server → route table ada 0.0.0.0/0 → igw-xxx. EC2 dapat public IP, boleh terima traffic dari internet.',
          ...
          },
          {
            ...
            ingat: '"Pintu keluar sahaja untuk subnet private — berbayar"',
            gunaUntuk: 'Private subnet instances access internet outbound-only',
            fungsi: 'Membolehkan instances dalam private subnet buat outbound connection ke internet (download patches, call APIs) tanpa dedah mereka kepada inbound connections dari internet. NAT Gateway duduk dalam PUBLIC subnet, ada Elastic IP. Private subnet route: 0.0.0.0/0 → NAT GW.',
            contohGuna: 'RDS dalam private subnet perlu download security patches → traffic pergi ke NAT GW (public subnet) → IGW → internet. Internet tak boleh initiate connection masuk ke RDS.',
            scenario: '"Private subnet EC2 perlu access internet tapi tak nak exposed dari luar" → NAT Gateway. Letak NAT GW dalam public subnet, route private subnet ke NAT GW.',
            keywords: ['NAT', 'outbound only', 'private subnet', 'Elastic IP', 'paid', 'no inbound'],
          },
          {
            shortName: 'Route Tables',
            fullName: 'VPC Route Tables',
            ingat: '"Peta jalan untuk traffic dalam VPC"',
            gunaUntuk: 'Control where network traffic is directed within VPC',
            fungsi: 'Setiap subnet mesti associate dengan satu route table. Route table ada rules yang tentukan ke mana traffic pergi. Public subnet ada route ke IGW. Private subnet ada route ke NAT GW atau tiada external route. Boleh ada main route table (default) dan custom route tables.',
            contohGuna: 'Public subnet route table: 172.16.0.0/16 → local, 0.0.0.0/0 → igw. Private subnet route table: 172.16.0.0/16 → local, 0.0.0.0/0 → nat-gw.',
            storageDetails: 'Local route → traffic dalam VPC (auto, tak boleh delete)\n0.0.0.0/0 → igw-xxx (public subnet route ke internet)\n0.0.0.0/0 → nat-xxx (private subnet route outbound)\n10.0.0.0/16 → pcx-xxx (VPC peering route)',
            detailsLabel: 'Common Routes',
            scenario: '"Subnet tak dapat access internet walaupun ada IGW" → check route table! Mesti ada 0.0.0.0/0 → IGW dan subnet mesti associate dengan route table tersebut.',
          ...
          },
          {
            ...
            ingat: '"Jambatan antara dua VPC"',
            gunaUntuk: 'Connect two VPCs to route traffic between them privately',
            fungsi: 'Membolehkan dua VPC communicate menggunakan private IP addresses seolah-olah dalam network yang sama. Boleh same account, cross-account, atau cross-region. NON-transitive — kalau A peer B dan B peer C, A tak boleh reach C secara automatik.',
            contohGuna: 'Production VPC (172.16.0.0/16) peer dengan Shared Services VPC (10.0.0.0/16) — team boleh access shared tools dari prod environment.',
            scenario: '"Connect dua VPC" → VPC Peering. Ingat: IP ranges tak boleh overlap! Dan non-transitive — kena buat peering connections berasingan kalau A nak reach C.',
          ...
          },
          {
            ...
            ingat: '"Highway terus ke AWS services — tanpa internet"',
            gunaUntuk: 'Access AWS services from VPC without going through internet',
            fungsi: 'Dua jenis: Gateway Endpoint (untuk S3 dan DynamoDB, free, guna route table) dan Interface Endpoint (untuk services lain guna AWS PrivateLink, ada private IP dalam subnet, berbayar). Traffic tak keluar ke internet langsung — lebih selamat dan murah.',
            contohGuna: 'EC2 dalam private subnet perlu access S3 → guna S3 Gateway Endpoint. Traffic terus ke S3 tanpa perlu NAT Gateway. Jimat kos!',
